...
class TD3:
    ...
    def eval_policy_fn(
        ...
        return true_returns, transitions
    ...
    def eval_qd_policy_fn(
        ...
        if reward_type == "final":
            last_index = (
                jnp.int32(jnp.nansum(1.0 - transitions.dones, axis=0, keepdims=True))
                - 1
            )
            fitnesses = jax.vmap(lambda x, y: x[y], in_axes=1, out_axes=1)(
                transitions.rewards, last_index
            )
        else:
            fitnesses = jnp.nansum(transitions.rewards, axis=0, keepdims=True)

        mean_fitness = jnp.nanmean(fitnesses, axis=-1)
        ...
        return fitnesses, bds, mean_fitness, mean_bd
