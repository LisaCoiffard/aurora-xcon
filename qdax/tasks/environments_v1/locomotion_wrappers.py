...
class XYPositionWrapper(QDEnv):
    ...
    def __init__(
        ...
        if hasattr(self.env, "sys"):
            ...
            # Add this line to make sure the aux_info is stored
            self.env.sys = QDSystem(self.env.sys.config)
        else:
            raise NotImplementedError(f"This wrapper does not support {env_name} yet.")
...
class XYPositionFeetInfoWrapper(QDEnv):
    ...
    def __init__(
        ...
        if env_name not in COG_NAMES.keys() or env_name not in FEET_NAMES.keys():
            raise NotImplementedError(f"This wrapper does not support {env_name} yet.")
...
class PassiveStandingWrapper(Wrapper):
    """
    ...
    def reset(self, rng: jp.ndarray) -> State:
        state = self.env.reset(rng)
        state.info["passive_descriptor"] = self._get_feet_contact(
            self.env.sys.info(state.qp)
        )
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        state = self.env.step(state, action)
        state.info["passive_descriptor"] = self._get_feet_contact(self.env.sys.aux_info)
        # update the reward (vector of full ant reward, no forward reward and forward reward only)
        new_reward = jnp.concatenate(
            (
                jp.array((state.reward,)),
                jp.array((state.reward - state.metrics[self._fd_reward_field],)),
...
class StandDescWrapper(QDEnv):
    ...
    def reset(self, rng: jp.ndarray) -> State:
        state = self.env.reset(rng)
        feet_contacts = self._get_feet_contact(self.env.sys.info(state.qp))
        state.info["state_descriptor"] = jnp.concatenate(
            (
                feet_contacts,
                self._get_z_torso_position(state.qp),
                self._get_z_torso_orientation(state.qp),
            ),
            axis=-1,
        )
        return state
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        state = self.env.step(state, action)
        feet_contacts = self._get_feet_contact(self.env.sys.aux_info)
        state.info["state_descriptor"] = jnp.concatenate(
            (
                feet_contacts,
                self._get_z_torso_position(state.qp),
    ...
    def _get_z_torso_position(self, qp: QP) -> jp.ndarray:
        z_pos = jnp.clip(
            qp.pos[self._cog_idx][2], a_min=self._minval[0], a_max=self._maxval[0]
        )
    ...
    def _get_z_torso_orientation(self, qp: QP) -> jp.ndarray:
        z_rot = jnp.clip(
            qp.ang[self._cog_idx][2], a_min=self._minval[1], a_max=self._maxval[1]
        )
        return jnp.expand_dims(
            z_rot,
            axis=-1,
