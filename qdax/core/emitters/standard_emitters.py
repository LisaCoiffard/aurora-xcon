...
class MixingEmitter(Emitter):
    ...
    def emit(
        ...
        if n_variation > 0:
            ...
            elif self._sampling_type == "fitness_prop":
                random_key, subkey = jax.random.split(random_key)
                x1 = repertoire.fitness_prop_sample(subkey, n_variation)
                x2 = repertoire.fitness_prop_sample(subkey, n_variation)
            else:
                raise ValueError(f"Unknown sampling type: {self._sampling_type}")
        ...
        if n_mutation > 0:
            if self._sampling_type == "random":
                ...
                random_key, subkey = jax.random.split(random_key)
                x1 = repertoire.fitness_prop_sample(subkey, n_mutation)
            else:
                raise ValueError(f"Unknown sampling type: {self._sampling_type}")
