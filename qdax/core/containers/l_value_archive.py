...
class UnstructuredRepertoire(flax.struct.PyTreeNode):
    ...
    def add(
        ...
        new_observations = self.observations.at[batch_of_indices.squeeze()].set(
            batch_of_observations.squeeze(axis=-1)
        )
    ...
    def init(
        ...
        repertoire = repertoire.add(  # type: ignore
    ...
    def extinction(self, remaining_prop: float, random_key) -> UnstructuredRepertoire:
        ...
        dummy_descriptors = jnp.full_like(self.descriptors, fill_value=0.0)
...
class UnstructuredRepertoirePassiveDesc(UnstructuredRepertoire):
    ...
    def add(
        ...
        batch_of_fitnesses = jnp.expand_dims(batch_of_fitnesses, axis=-1)
        batch_of_observations = jnp.expand_dims(batch_of_observations, axis=-1)
        batch_of_passive_descriptors = jnp.expand_dims(
            batch_of_passive_descriptors, axis=-1
        )
        ...
        batch_of_fitnesses = batch_of_fitnesses.at[sorted_bds].get()
        batch_of_observations = batch_of_observations.at[sorted_bds].get()
        batch_of_passive_descriptors = batch_of_passive_descriptors.at[sorted_bds].get()
        not_novel_enough = not_novel_enough.at[sorted_bds].get()
        ...
        new_passive_descriptors = self.passive_descriptors.at[
            batch_of_indices.squeeze()
        ].set(batch_of_passive_descriptors.squeeze())

        return UnstructuredRepertoirePassiveDesc(
            genotypes=new_grid_genotypes,
            fitnesses=new_fitnesses.squeeze(),
            descriptors=new_descriptors.squeeze(),
            passive_descriptors=new_passive_descriptors.squeeze(),
    ...
    def init(
        ...
        default_descriptors = jnp.zeros(shape=(max_size, descriptors.shape[-1]))
        default_passive_descriptors = jnp.zeros(
            shape=(max_size, passive_descriptors.shape[-1])
        )
        ...
        repertoire = UnstructuredRepertoirePassiveDesc(
            ...
            fitnesses=default_fitnesses,
            descriptors=default_descriptors,
            passive_descriptors=default_passive_descriptors,
            observations=default_observations,
            l_value=l_value,
            max_size=max_size,
        )
        ...
        return repertoire.add(  # type: ignore
            genotypes, descriptors, passive_descriptors, fitnesses, observations
        )
