...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    def init(
        ...
        new_repertoire, condition = repertoire.add(
            genotypes, descriptors, fitnesses, extra_scores
        )

        return new_repertoire, condition  # type: ignore
    ...
    def extinction(self, remaining_prop: float, random_key) -> MapElitesRepertoire:
        ...
        final_mask = (
            jnp.zeros(num_individuals, dtype=bool).at[selected_indices].set(True)
        )

        # Create dummy values for non-selected entries
        dummy_fitnesses = jnp.full_like(self.fitnesses, fill_value=-jnp.inf)
        dummy_descriptors = jnp.zeros_like(self.descriptors)
        ...
        def mask_genotype(x):
            ...
            return jnp.where(expanded_mask, x, jnp.zeros_like(x))
