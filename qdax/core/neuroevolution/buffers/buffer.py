...
class PassDescQDTransition(QDTransition):
    """Stores data corresponding to a transition collected by a QD algorithm with auxiliary information for plotting a passive archive."""

    passive_desc: StateDescriptor
    next_passive_desc: StateDescriptor

    @property
    def passive_descriptor_dim(self) -> int:
        """
        Returns:
            the dimension of the state descriptors.

        """
        return self.passive_desc.shape[-1]  # type: ignore
    ...
    def from_flatten(
        ...
        action_dim = transition.action_dim
        desc_dim = transition.state_descriptor_dim
        passive_desc_dim = transition.passive_descriptor_dim
        ...
        passive_desc = flattened_transition[
            :,
            (2 * obs_dim + 3 + action_dim + 2 * desc_dim) : (
                2 * obs_dim + 3 + action_dim + 2 * desc_dim + passive_desc_dim
            ),
        ]
        next_passive_desc = flattened_transition[
            :,
            (2 * obs_dim + 3 + action_dim + 2 * desc_dim + passive_desc_dim) : (
                2 * obs_dim + 3 + action_dim + 2 * desc_dim + 2 * passive_desc_dim
            ),
        ]
        return cls(
            obs=obs,
            ...
            next_passive_desc=next_passive_desc,
        )
    ...
    def init_dummy(  # type: ignore
        ...
        dummy_transition = QDTransition(
            ...
            state_desc=jnp.zeros(shape=(1, descriptor_dim)),
            next_state_desc=jnp.zeros(shape=(1, descriptor_dim)),
            passive_desc=jnp.zeros(shape=(1, passive_descriptor_dim)),
            next_passive_desc=jnp.zeros(shape=(1, passive_descriptor_dim)),
        )
        return dummy_transition
