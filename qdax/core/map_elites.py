...
class MAPElites:
    ...
    def update(
        ...
        repertoire, _ = repertoire.add(genotypes, descriptors, fitnesses, extra_scores)
...
class MAPElitesPassive:
    """Core elements of the MAP-Elites algorithm which maintains a passive repertoire of 
    solutions according to another hand-coded BD characterization.
    ...
    def init(
        ...
        Returns:
            An initialized MAP-Elites repertoire and passive repertoire,
            with the initial state of the emitter, and a random key.
        """
        # score initial genotypes
        fitnesses, descriptors, extra_scores, random_key = self._scoring_function(
            genotypes, random_key
        ...
        # init the repertoire
        repertoire, condition = MapElitesRepertoire.init(
            genotypes=genotypes,
            fitnesses=fitnesses,
            descriptors=descriptors,
            centroids=centroids,
        ...
        # setting -inf fitness to all individuals that were not added to the main repertoire
        passive_fitnesses = jnp.where(
            condition.squeeze(axis=-1), extra_scores["passive_fitnesses"], -jnp.inf
        )
    ...
    def update(
        ...
        Returns:
            the updated MAP-Elites repertoire
            the updated passive repertoire
            the updated (if needed) emitter state
            metrics about the updated repertoire
            a new jax PRNG key
        """
        ...
        # add genotypes in the repertoire
        repertoire, condition = repertoire.add(
        ...
        passive_repertoire, _ = passive_repertoire.add(
            genotypes, passive_descriptors, passive_fitnesses, extra_scores
        )
